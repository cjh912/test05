...
import com.example.TF.repository.Movie_qna_board_repository;

@Repository
public class Movie_qna_board_DAO {

	@Autowired
	Movie_qna_board_repository qna_repository;
	...
	public int qna_board_modify(Movie_qna_board board) {
		// 1. 기존 데이터 가져오기
		Movie_qna_board board_old = qna_repository.findById(board.getSeq()).orElse(null);
		int result = 0;
		if (board_old != null) {
			// 2. 수정
			board.setLogtime(board_old.getLogtime());
			// 3. 저장
			Movie_qna_board board_result = qna_repository.save(board);
	...
	public int qna_board_delete(int seq) {
		// 1. 기존 데이터 가져오기
		Movie_qna_board board = qna_repository.findById(seq).orElse(null);
		int result = 0;
